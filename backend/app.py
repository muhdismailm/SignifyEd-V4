...
AWS_S3_BUCKET_URL = os.environ.get("AWS_S3_BUCKET_URL", "https://signifyedkeypoints.s3.eu-north-1.amazonaws.com")
...
def translate_malayalam_audio(audio_file):
    """Translate Malayalam speech to English using Sarvam Saaras v3."""
    api_key = get_sarvam_api_key()
    if not api_key:
        raise RuntimeError("SARVAM_API_KEY is not configured on the backend")

    filename = secure_filename(audio_file.filename or "malayalam-audio.webm")
    mime_type = audio_file.mimetype or "audio/webm"
    response = requests.post(
        SARVAM_STT_URL,
        headers={"api-subscription-key": api_key},
        files={"file": (filename, audio_file.stream, mime_type)},
        data={"model": "saaras:v3", "mode": "translate", "language_code": "ml-IN"},
        timeout=60,
    )
    if not response.ok:
        try:
        ...
        raise RuntimeError(f"Sarvam AI request failed ({response.status_code}): {detail}")

    payload = response.json()
    transcript = str(payload.get("transcript") or "").strip()
    if not transcript:
        # Log response metadata only: this helps diagnose recognition failures
        # without logging the API key or the user's audio.
        print(
            "[SARVAM] Empty translation. "
            f"request_id={payload.get('request_id')}, "
            f"language_code={payload.get('language_code')}, "
            f"language_probability={payload.get('language_probability')}, "
            f"response_keys={list(payload.keys())}"
        )
        raise ValueError(
            "No Malayalam speech was detected in the recording. "
            "Record for at least 3 seconds, speak clearly, and try again."
        )
    return transcript


def transcribe_malayalam_audio(audio_file):
    """Transcribe a PCM WAV Malayalam recording with Sarvam Saaras v3."""
    api_key = get_sarvam_api_key()
    if not api_key:
        raise RuntimeError("SARVAM_API_KEY is not configured on the backend")
    ...
    if not response.ok:
        try:
            ...
            detail = response.text
        raise RuntimeError(f"Sarvam AI transcription failed ({response.status_code}): {detail}")

    payload = response.json()
    transcript = str(payload.get("transcript") or "").strip()
    if not transcript:
        print(
            "[SARVAM] Empty Malayalam transcription. "
            f"request_id={payload.get('request_id')}, "
            f"language_code={payload.get('language_code')}, "
            f"language_probability={payload.get('language_probability')}, "
            f"response_keys={list(payload.keys())}"
        ...
        raise ValueError(
            "Sarvam could not recognize Malayalam in this audio. "
            "Check that the correct microphone is selected, then speak clearly for at least 3 seconds."
        )
    return transcript


def translate_malayalam_text(input_text):
    """Translate Malayalam text to English using Sarvam's text translation API."""
    api_key = get_sarvam_api_key()
    if not api_key:
        raise RuntimeError("SARVAM_API_KEY is not configured on the backend")
    ...
    if not response.ok:
        try:
            ...
            detail = response.text
        raise RuntimeError(f"Sarvam AI translation failed ({response.status_code}): {detail}")

    translated_text = str(response.json().get("translated_text") or "").strip()
    if not translated_text:
        raise ValueError("Sarvam AI returned an empty English translation for this Malayalam text.")
...
@app.route("/transcribe_malayalam_audio", methods=["POST"])
def transcribe_malayalam_audio_route():
    """Return Malayalam text for a microphone WAV capture without translating it."""
    try:
        if "audio" not in request.files:
            return jsonify({"error": "Audio file missing"}), 400

        transcript = transcribe_malayalam_audio(request.files["audio"])
        return jsonify({"transcript": transcript, "source_language": "ml-IN"})
...
@app.route("/process_malayalam_text", methods=["POST"])
def process_malayalam_text():
    """Translate typed Malayalam to English, then produce ISL keypoints."""
    try:
        data = request.get_json(silent=True)
        input_text = (data or {}).get("text", "").strip()
        if not input_text:
            return jsonify({"error": "Malayalam text is required"}), 400

        english_translation = translate_malayalam_text(input_text)
        result = process_text_pipeline(english_translation)
        combined_data, missing = combine_sentence_keypoints(result["isl_gloss"])
        return jsonify({
            **result,
            "source_language": "ml-IN",
            "original_malayalam": input_text,
...
@app.route("/process_malayalam_audio", methods=["POST"])
def process_malayalam_audio():
    """Translate Malayalam microphone audio to English, then produce ISL keypoints."""
    try:
        if "audio" not in request.files:
            return jsonify({"error": "Audio file missing"}), 400

        english_translation = translate_malayalam_audio(request.files["audio"])
        result = process_text_pipeline(english_translation)
        combined_data, missing = combine_sentence_keypoints(result["isl_gloss"])
