...
//    MEDIAPIPE HELPERS
// ───────────────────────────────────────────────────────────── */
// // For pose/arm landmarks: negate x (mirror) and y (MediaPipe y-down → Three.js y-up)
...
// // For hand landmarks: negate x (mirror) and z (flip depth so palm faces forward)
// // Negating y here would flip the palm to face backward, so we leave y as-is.
// function mpHandToWorld(p: number[]): THREE.Vector3 {
//   return new THREE.Vector3(p[0], p[1], p[2]);
// }
// function mpHandDir(a: number[], b: number[]): THREE.Vector3 {
//   return mpHandToWorld(b).sub(mpHandToWorld(a)).normalize();
// }
...
//     rotateBoneToward(b1, mpHandDir(lm[mcp], lm[pip]));
//     rotateBoneToward(b2, mpHandDir(lm[pip], lm[dip]));
//     rotateBoneToward(b3, mpHandDir(lm[dip], lm[tip]));
//   }

//   function driveHand(hand: number[][], side: "l" | "r") {
//     if (!hand || hand.length < 21) return;
//     const s = side;
//     rotateBoneToward(BONES[`${s}_hand`], mpHandDir(hand[0], hand[9]));
...
//   isPlaying, onPlay, onPause, onReplay, frameIndex, total,speed,onSpeedChange
// }: {
//   isPlaying: boolean;
//   onPlay: () => void;
//   onPause: () => void;
...
// /* ─────────────────────────────────────────────────────────────
//    BONE CHILD MAP
//    For bones with multiple children (e.g. Hand → 5 fingers),
//    we must pick ONE child to define the bone's "forward" direction.
//    This map picks the anatomically correct child for each bone.
// ───────────────────────────────────────────────────────────── */
...
// /* ─────────────────────────────────────────────────────────────
//    MIXAMO BONE ALIASES  (shorthand → Mixamo bone name)
// ───────────────────────────────────────────────────────────── */
...
// /* ─────────────────────────────────────────────────────────────
//    MEDIAPIPE → THREE.JS  (unified coordinate transform)
//    MediaPipe world landmarks:
//      x = right of subject (screen left)
//      y = down
//      z = toward camera
//    Three.js:
//      x = right
//      y = up
//      z = toward camera
//    Transform: negate x (mirror), negate y (flip vertical), negate z (depth)
// ───────────────────────────────────────────────────────────── */
...
//   /* ── Cache bones & rest quaternions on load ────────────── */
//   useEffect(() => {
//     scene.traverse((obj) => {
//       if (!(obj as THREE.Bone).isBone) return;
//       const bone = obj as THREE.Bone;
...
//   /* ── Get LIVE world-space bone direction ────────────────
//      Uses BONE_CHILD_MAP to find the correct child.
//      Must be called AFTER parent bones have been rotated
//      (top-down order) and after resetToRest.
//   ──────────────────────────────────────────────────────── */
...
//   /* ── Rotate a single bone so its "forward" matches target ─ */
...
//     // Compute delta quaternion in world space
//     const dot = currentDir.dot(targetDir);
//     let delta: THREE.Quaternion;

//     if (dot < -0.9999) {
//       // Near 180° — pick an arbitrary perpendicular axis
...
//   /* ── Drive an entire hand (wrist + 5 fingers) ─────────── */
//   function driveHand(hand: number[][], side: "l" | "r") {
//   if (!hand || hand.length < 21) return;
//   const s = side;

//   // Wrist orientation: wrist(0) → middle-finger MCP(9)
//   rotateBoneToward(BONES[`${s}_hand`], mpDir(hand[0], hand[9]));

//   // Thumb
//   driveFinger(hand, 1, 2, 3, 4,
//     BONES[`${s}_thumb1`], BONES[`${s}_thumb2`], BONES[`${s}_thumb3`]);
//   // Index
//   driveFinger(hand, 5, 6, 7, 8,
//     BONES[`${s}_index1`], BONES[`${s}_index2`], BONES[`${s}_index3`]);
//   // Middle
//   driveFinger(hand, 9, 10, 11, 12,
//     BONES[`${s}_mid1`], BONES[`${s}_mid2`], BONES[`${s}_mid3`]);
//   // Ring
//   driveFinger(hand, 13, 14, 15, 16,
//     BONES[`${s}_ring1`], BONES[`${s}_ring2`], BONES[`${s}_ring3`]);
//   // Pinky
//   driveFinger(hand, 17, 18, 19, 20,
//     BONES[`${s}_pinky1`], BONES[`${s}_pinky2`], BONES[`${s}_pinky3`]);
// }
...
//     // ──── STEP 1: Reset all bones to rest pose ────────────
//     resetToRest();

//     // ──── STEP 2: Drive arms (top-down order) ─────────────
//     // MediaPipe pose landmarks:
//     //   11 = left shoulder,  12 = right shoulder
//     //   13 = left elbow,     14 = right elbow
//     //   15 = left wrist,     16 = right wrist
//     if (pose && pose.length >= 17) {
//       // Right arm (MediaPipe left = avatar right in mirrored view)
//       rotateBoneToward(BONES.r_upperarm, mpDir(pose[11], pose[13]));
//       rotateBoneToward(BONES.r_forearm,  mpDir(pose[13], pose[15]));
...
//     // ──── STEP 3: Drive hands (after arms are set) ────────
//     if (left)  driveHand(left,  "l");
//     if (right) driveHand(right, "r");
//   });
...
//     function drawHand(
//       c: CanvasRenderingContext2D, lm: number[][],
//       wx: number, wy: number, lineColor: string, dotColor: string
//     ) {
//       if (!lm || lm.length < 21) return;
//       c.lineCap = "round";
//       c.lineJoin = "round";
...
//       onTouchEnd={stopDrag}
//       onDoubleClick={handleDoubleClick}
//       style={{ width: "100%", height: "100%", display: "block", borderRadius: 12, cursor: "grab" }}
//     />
//   );
// }
...
//   };
//   return (
//     <div style={{ padding: "10px 14px", background: "rgba(15,23,42,0.85)", borderTop: "1px solid rgba(99,102,241,0.12)" }}>
//       {/* Progress bar */}
//       <div style={{ height: 4, background: "rgba(99,102,241,0.15)", borderRadius: 2, marginBottom: 10, overflow: "hidden" }}>
//        <div style={{ width: `${pct}%`, height: "100%", background: "linear-gradient(90deg,#818cf8,#6366f1)", borderRadius: 2, transition: "width 0.08s" }} />
//       </div>
...
//         <button
//           onClick={onReplay}
//           style={{ ...base, width: 36, height: 36, borderRadius: "50%", background: "rgba(99,102,241,0.12)", color: "#a5b4fc", fontSize: 16 }}
...
//           <button
//             onClick={onPause}
//             style={{ ...base, width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg,#6366f1,#818cf8)", color: "#fff", fontSize: 18, boxShadow: "0 0 16px rgba(99,102,241,0.4)" }}
...
//         <span style={{ color: "rgba(148,163,184,0.7)", fontSize: 12, fontFamily: "monospace", minWidth: 70, textAlign: "center" }}>
// {total > 0 ? `${frameIndex + 1} / ${total}` : "no data"}        </span>
//       </div>
//     </div>
//   );
// }
...
// {/* VIEWER */}
// <div style={{ position: "relative", width: "100%", height: 315, overflow: "hidden" }}>        
//        {/* Avatar */}
// <div style={{ 
//   position: "absolute",
//   inset: 0,
//   display: activeTab === "avatar" ? "block" : "none"
// }}>        <Canvas 
//   camera={{ position: [0, 1, 3], fov: 45 }} 
//   style={{ background: "#080d1a", width: "100%", height: "100%" }}  // ← add width/height
// >
//             <ambientLight intensity={0.6} />
...
function SkeletonViewer({
  ...
  const draw = useCallback((frameIdx: number) => {
    ...
    if (pose && pose.length >= 11) {
      ...
      for (const eye of [lEye, rEye]) {
        if (!eye) continue;
        ctx.beginPath(); ctx.arc(eye[0], eye[1], 3.5, 0, Math.PI * 2);
        ctx.fillStyle = "#a5b4fc"; ctx.shadowColor = "#818cf8"; ctx.shadowBlur = 8; ctx.fill(); ctx.shadowBlur = 0;
      }
      for (const ear of [lEar, rEar]) {
        if (!ear) continue;
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    if (e.touches.length === 2) {
      dragRef.current.lastDist = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY);
    } else {
      dragRef.current = { active: true, lastX: e.touches[0].clientX, lastY: e.touches[0].clientY, lastDist: 0 };
    }
  }, []);
...
function PlaybackControls({
  isPlaying, onPlay, onPause, onReplay,
  frameIndex, total,
  speed, onSpeedChange,
}: {
  isPlaying: boolean;
          ...
          {SPEED_OPTIONS.map((s) => {
            ...
            return (
              <button
                ...
                onMouseEnter={e => {
                  if (!active) e.currentTarget.style.background = "rgba(99,102,241,0.22)";
                }}
                onMouseLeave={e => {
                  if (!active) e.currentTarget.style.background = "rgba(99,102,241,0.1)";
                }}
              >
                {s}×
              </button>
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", width: "100%", height: 315, overflow: "hidden" }}>
        {/* Avatar */}
        <div style={{ position: "absolute", inset: 0, display: activeTab === "avatar" ? "block" : "none" }}>
          <Canvas camera={{ position: [0, 1, 3], fov: 45 }} style={{ background: "#080d1a", width: "100%", height: "100%" }}>
            <ambientLight intensity={0.6} />
            <directionalLight position={[2, 3, 4]} intensity={1} />
            <AnimatedAvatar
              keypoints={keypoints}
        ...
        {/* Skeleton */}
        <div style={{ position: "absolute", inset: 0, display: activeTab === "skeleton" ? "flex" : "none", alignItems: "center", justifyContent: "center", height: "100%" }}>
          {keypoints.length > 0 ? (
            <SkeletonViewer
              keypoints={keypoints}
              fps={fps}
